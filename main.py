...
def train(config, model, optimizer, x, y, use_len, stride):
    model.train()
    optimizer.zero_grad()
    y_out = step_model(config, model, x[:, :use_len], start[:, :use_len])
    loss = 0.0
    
    for n in range(use_len - stride, use_len):
        loss += criterion(y_out[:, n, :], y[:, n+1])
    loss.backward()
    print(f"loss: {loss / stride} use_len: {use_len}")
    optimizer.step()
    print("    VALID")
    generate_sentence(config, model, "Sapporo")
    print("Y_OUT")
    print(bpe_tokenizer.decode(torch.argmax(y_out[0], dim=-1).tolist()))
    return loss.item() / stride
...
def generate_sentence(config, model, source, input_more="", sentence_len=50):
    ...
    with torch.no_grad():
        ...
        print(f"    {bpe_tokenizer.decode(y_decode[0].tolist())}")
...
for i in tqdm(range(1000)):
    use_len = random.randint(stride, x.size(1)-1)
    loss_result = train(config, model, optimizer, x, y, use_len, stride)
